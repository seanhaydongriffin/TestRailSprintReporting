...
namespace TestRailSprintReporting
{
    class Program
    {
        ...
        {
            //var xml5 = SharedProject.File.read("C:\\dwn\\fred.xml");
            ////var build_plan_runs5 = Xml.GetNode(xml5, "//*[local-name()='id']");
            //var build_plan_runs5 = Xml.GetNode(xml5, "milestone/runs/plan/id[text()='R5111']/.."); //.Cast<XmlNode>().Where(n => n.Attributes["id"].InnerText == "R5111");

            //foreach (XmlNode plan5 in build_plan_runs5)
            //{
            //    var sss = plan5.Attributes["id"].InnerText;
            //    var plan_id = plan5.GetChildNode("id").InnerText;
            //    var plan_name = plan5.GetChildNode("name").InnerText;
            //}
            ...
            var confluence_build_page_table = new Dictionary<string, DataTable>();
            var confluence_sprint_page_table = new Dictionary<string, DataTable>();
            ...
            var testrail_sprint_run_id = new Dictionary<string, string>();

            var testrail_build_run_id = new Dictionary<string, string>();

            var daily_untested_count = new Dictionary<string, int?>();
            var daily_failed_count = new Dictionary<string, int?>();
            var build_plans = new Dictionary<string, IEnumerable<JToken>>();
            var quarter_milestones_export = new Dictionary<int, string>();
            ...
            foreach (XmlNode TestProject in AppConfig.GetSectionGroup("TestProjects").GetSectionGroups())
            {
                ...
                var TestProjectConfluenceSprintsRootKey = TestProject.GetAttributeValue("ConfluenceSpace") + "-" + TestProject.GetAttributeValue("ConfluenceSprintsPage") + "-" + TestProject.GetAttributeValue("Team");

                if (!confluence_sprint_page_table.ContainsKey(TestProjectConfluenceSprintsRootKey))
                {
                    ...
                    confluence_sprint_page_table.Add(TestProjectConfluenceSprintsRootKey, results_table);
                }
                ...
                // Locate all quarter milestones that containing the full list of project builds

                var builds_milestones = milestones.SelectTokens("$..milestones[?(@.name == 'Builds')].milestones");
                //var build_milestone = builds_milestone.SelectToken("$..milestones[?(@.name == '" + BuildName + "')]");

                // Get all project plans

                Log.WriteLine("Prj " + project_num + " of " + AppConfig.GetSectionGroup("TestProjects").GetSectionGroups().Count + " \"" + TestProjectName + "\" getting the plans ...");
                var plans2 = (JObject)TestRailAPIClient.SendGet("get_plans/" + TestProjectId);

                // Locate all project plans that include each project build

                foreach (var build_milestone in builds_milestones.Children())
                {
                    var build_name = build_milestone["name"].ToString();

                    Log.WriteLine("Looking for plans containing Build \"" + build_name + "\" ...");
                    var build_plans2 = plans2.SelectTokens("plans").Children().Where(x => x["name"].ToString().Contains(build_name));

                    var TestProjectConfluenceBuildsRootKey = TestProject.GetAttributeValue("ConfluenceSpace") + "-" + TestProject.GetAttributeValue("ConfluenceBuildsPage") + "-" + TestProject.GetAttributeValue("Team") + "-" + build_name;

                    if (!confluence_build_page_table.ContainsKey(TestProjectConfluenceBuildsRootKey))
                    {
                        var results_table = new DataTable();
                        results_table.Columns.Add("TestProject", typeof(string));
                        results_table.Columns.Add("TestRun", typeof(string));
                        ...
                        results_table.Columns.Add("TestTestedOn", typeof(string));
                        results_table.Columns.Add("TestAllDefects", typeof(string));
                        confluence_build_page_table.Add(TestProjectConfluenceBuildsRootKey, results_table);
                    }




                    // locate the parent (quarter) milestone for each build plan (such that we can later query this for performance reasons)

                    foreach (var build_plan in build_plans2)
                    {
                        Log.WriteLine("Looking for Milestones containing Plan \"" + build_plan["name"] + "\" ...");
                        var build_plan_sprint_milestone = milestones.SelectToken("$..milestones[?(@.id == " + build_plan["milestone_id"] + ")]");
                        var build_plan_quarter_milestone = milestones.SelectToken("$..milestones[?(@.id == " + build_plan_sprint_milestone["parent_id"] + ")]");

                        if (!quarter_milestones_export.ContainsKey((int)build_plan_quarter_milestone["id"]))
                        {
                            Log.WriteLine("Prj " + project_num + " of " + AppConfig.GetSectionGroup("TestProjects").GetSectionGroups().Count + " exporting Milestone \"" + build_plan_quarter_milestone["name"] + "\" ...");
                            var xml2 = TestRailWebClient.SendGet("milestones/export/" + (int)build_plan_quarter_milestone["id"]);
                            //SharedProject.File.overwrite("C:\\dwn\\fred.xml", xml2);
                            quarter_milestones_export.Add((int)build_plan_quarter_milestone["id"], xml2);
                        }

                        var build_plan_runs = Xml.GetChildNodes(quarter_milestones_export[(int)build_plan_quarter_milestone["id"]], "milestone/runs/plan/id[text()='R" + build_plan["id"] + "']/../runs/run");
                        ...
                        foreach (XmlNode run in build_plan_runs)
                        {
                            var run_id = run.GetChildNode("id").InnerText;
                            var run_name = run.GetChildNode("name").InnerText;
                            testrail_build_run_id.put(build_name + "-" + run_name.ToString().Replace(" Functional", "").Replace(" Regression", "").Trim(), run_id.Replace("R", ""));
                            ...
                            foreach (XmlNode test in tests)
                            {
                                ...
                                for (var day = current_sprint_start; day.Date <= current_sprint_end; day = day.AddDays(1))
                                {
                                    ...
                                    if (test_date_outcome.get(day_str).Equals("Untested"))
                                        ...
                                        daily_untested_count.Increment(build_plan["name"] + "~" + day_str);

                                    if (test_date_outcome.get(day_str).Equals("Failed"))

                                        daily_failed_count.Increment(build_plan["name"] + "~" + day_str);

                                    prev_day_result = test_date_outcome.get(day_str);
                                }

                                confluence_build_page_table[TestProjectConfluenceBuildsRootKey].Rows.Add(TestProjectName.ToString().Replace(" Assessment", "").Trim(), run_name.ToString().Replace(" Functional", "").Replace(" Regression", "").Trim(), testcaseid.Replace("C", "").ToInt(), auto_script_ref, testtitle, teststatus, tested_on, all_defects);

                                // Tally the test results for later pie charting

                                //testrail_plan_type_status_count.Increment(build_plan["name"].ToString().Replace(current_sprint_name, "").Trim() + "-" + teststatus);
                            }
                        }
                    ...
                    //build_plans.Add(build_name, build_plans2);


                    //confluence_build_page_table[TestProjectConfluenceBuildsRootKey].Rows.Add(TestProjectName.ToString().Replace(" Assessment", "").Trim(), plan_name, run_name.ToString().Replace(" Functional", "").Replace(" Regression", "").Trim(), testcaseid.Replace("C", "").ToInt(), auto_script_ref, testtitle, teststatus, tested_on, all_defects);
                }
                ...
                foreach (XmlNode plan in plans)
                {
                    ...
                    foreach (XmlNode run in runs)
                    {
                        var run_id = run.GetChildNode("id").InnerText;
                        var run_name = run.GetChildNode("name").InnerText;
                        testrail_sprint_run_id.put(run_name.ToString().Replace(" Functional", "").Replace(" Regression", "").Trim(), run_id.Replace("R", ""));
                        var tests = run.GetChildNodes("sections/section/tests/test");
                        ...
                        foreach (XmlNode test in tests)
                        {
                            ...
                            confluence_sprint_page_table[TestProjectConfluenceSprintsRootKey].Rows.Add(TestProjectName.ToString().Replace(" Assessment", "").Trim(), plan_name, run_name.ToString().Replace(" Functional", "").Replace(" Regression", "").Trim(), testcaseid.Replace("C", "").ToInt(), auto_script_ref, testtitle, teststatus, tested_on, all_defects);
            ...
            // for each build confluence page to update

            foreach (var each_confluence_page_table in confluence_build_page_table)
            {
                ...
                var build_name = each_confluence_page_table.Key.Split('-')[3];

                // Check if the build page exists in Confluence (under the provided root page provided in the config)

                var confluence_child_page = (JObject)ConfluenceClient.SendGet(confluence_parent_page_key + "/child/page?limit=200");

                var build_page_key = confluence_child_page.SelectToken("$..results[?(@.title == '" + team_name + " " + build_name + "')].id");

                if (build_page_key == null)
                {
                    // Create the Build Page

                    Log.WriteLine("Confluence Page \"" + team_name + " " + build_name + "\" creating ...");

                    var confluence_create_page_json = new
                    {
                        type = "page",
                        title = team_name + " " + build_name,
                        @space = new
                        {
                    ...
                    build_page_key = result["id"];
                }

                confluence_child_page = (JObject)ConfluenceClient.SendGet(build_page_key + "/child/page?limit=200");

                var build_qa_page_key = confluence_child_page.SelectToken("$..results[?(@.title == '" + team_name + " " + build_name + " QA Test Results')].id");

                if (build_qa_page_key == null)
                {
                    // Create the Build QA Page

                    Log.WriteLine("Confluence Page \"" + team_name + " " + build_name + " QA Test Results\" creating ...");

                    var confluence_create_page_json = new
                    {
                        type = "page",
                        title = team_name + " " + build_name + " QA Test Results",
                        @space = new
                        {
                            key = confluence_space_key
                        },
                ...
                results_table_view.Sort = "TestProject, TestRun";
                ...
                foreach (DataRowView row in results_table_view)
                {
                    ...
                    table_rows.Add(
                        ...
                        new Hyperlink(AppConfig.Get("TestRailUrl") + "/index.php?/runs/view/" + testrail_build_run_id.get(build_name + "-" + row["TestRun"].ToString()), row["TestRun"].ToString()),
                        new TableCell(
                ...
                var table = new Table("full-width").Add(
                    new TableColumnGroup(70, 130, 150, 280, 40, 70, 80)).Add(
                    new TableHead("Client / Proj", "Test Level - Run", "[ID] Test Name", "Test Title", "Status", "Tested On", "All Defects")).Add(
...
//                confluence_page_storage_str = want_to_know_more + "<h5 style=\"text-align: center;\">Status Charts by Test Type</h5>" + pie_chart_section.ToStringDisableFormatting() + confluence_page_storage_str;
                ...
                Log.WriteLine("Confluence Page \"" + build_qa_page_key + "\" getting the version ...");
                var confluence_page = (JObject)ConfluenceClient.SendGet(build_qa_page_key + "?expand=version");
                ...
                Log.WriteLine("Confluence Page \"" + build_qa_page_key + "\" updating ...");
                var pp = (JToken)ConfluenceClient.SendPut(build_qa_page_key.ToString(), confluence_json);
                int i = 0;
            ...
            // for each sprint confluence page to update

            foreach (var each_confluence_page_table in confluence_sprint_page_table)
            {
                var confluence_space_key = each_confluence_page_table.Key.Split('-')[0];
                var confluence_parent_page_key = each_confluence_page_table.Key.Split('-')[1];
                var team_name = each_confluence_page_table.Key.Split('-')[2];
                ...
                foreach (DataRowView row in results_table_view)
                {
                    ...
                    table_rows.Add(
                        ...
                        new Hyperlink(AppConfig.Get("TestRailUrl") + "/index.php?/runs/view/" + testrail_sprint_run_id.get(row["TestRun"].ToString()), row["TestRun"].ToString()),
                        new TableCell(
                            new Anchor("C" + row["TestAutoID"]),
                            "[",
                            new Hyperlink(AppConfig.Get("TestRailUrl") + "/index.php?/cases/view/" + row["TestAutoID"], "C" + row["TestAutoID"]),
